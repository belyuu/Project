...
#define NOMINMAX  // Отключаем макросы min/max из Windows, т.к. у меня конфликт с отображением.
#ifdef _WIN32
#include <windows.h>
#endif
...
private:
    ...
    void calculateWidths(int& wName, int& wCapital, int& wPop, int& wArea, int& wIndustry, int& wReligion) const {
        ...
        for (const auto& c : countries) {
            wName = std::max(wName, (int)c.name.length());
            wCapital = std::max(wCapital, (int)c.capital.length());
            wPop = std::max(wPop, (int)to_string(c.population).length());
            wArea = std::max(wArea, (int)to_string(c.area).length());
            wIndustry = std::max(wIndustry, (int)c.industry.length());
            wReligion = std::max(wReligion, (int)c.religion.length());
        }
...
public:
    ...
    Database filterByDensity(double densityThreshold) const {
        ...
        for (const auto& c : countries) {
            if (c.area > 0) {
                double density = static_cast<double>(c.population) / c.area;
...
int main() {
    ...
    Database db;
    string defaultFile = "countries.txt";

    if (!db.loadFromFile(defaultFile)) {
        cout << "\nВНИМАНИЕ: Файл \"" << defaultFile << "\" не найден.\n";
        cout << "Вы можете:\n";
        cout << "  - поместить файл в текущую папку и загрузить его через пункт меню 1,\n";
        cout << "  - или добавлять записи вручную (пункт 3).\n";
        cout << "Продолжаем работу с пустой базой данных.\n\n";
    }
